<template>
    <section class="v-cal-content">
        <div class="v-cal-weekdays">
            <div class="v-cal-weekday-item">
                <span>{{ activeDate.format('ddd') }}</span>
                <span>{{ activeDate.format('DD/MM') }}</span>
            </div>
        </div>
            ...
            <div class="v-cal-days__wrapper">
                <div class="v-cal-day v-cal-day--day" :class="{ 'is-today': day.isToday }" v-if="day !== null">
                    ...
                    <div class="v-cal-day__hour-block"
                         ...
                         :class="[ time.isSame(now, 'hour') ? 'is-now' : '', hourClass ]" v-for="(time, index) in day.availableTimes" :key="index">
...
<script>
    ...
    export default {
        ...
        computed: {
          dayEvents(events, time) {
              return events.filter((event) => event.startTime && time.hours() === event.startTime.hours());
