...
<!--      <div class="v-cal-times">-->
<!--          &lt;!&ndash;        <div class="v-cal-hour all-day">++{{ allDayLabel }}</div>&ndash;&gt;-->
<!--          <div class="v-cal-hour"></div>-->
<!--      </div>-->
...
<!--                   >-->
<!--                  <span class="v-cal-day__hour-block-fill">{{ 222 }}</span>-->
...
<!--          </div>-->
          ...
          <div class="v-cal-day__hour-block"
               ...
               :class="[ time.hour() === now.hour() ? 'is-now' : '', hourClass ]" v-for="(time, index) in day.availableTimes" :key="index">
            <span class="v-cal-day__hour-block-fill">{{ time | formatTime(use12) }}</span>
            <div class="v-cal-day__hour-content">
              <div class="v-cal-event-list">

                <event-item
                        v-for="(event, index) in dayEvents(day, time)"
                        :key="index"
                        :event="event"
                        :use12="use12"
>
<!--                  <event-item-->
<!--                      v-for="(event, index) in weekEvents(day.events, time)"-->
<!--                      :key="index"-->
<!--                      :event="event"-->
<!--                      :use12="use12"-->
<!--                  >-->
...
<script>
    ...
    export default {
        ...
        computed: {
            // weekEvents(day, time) {
            //     console.log(time)
            //     return (day.events.length && typeof(time) !== 'undefined') ? day.events.filter((event) => event.startTime && time.hours() === event.startTime.hours()) : [];
            //     // return event.startTime && time.hours() === event.startTime.hours()
            // }
        },
        mounted() {
            this.buildCalendar();
